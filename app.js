...
function ffTrig(o) {   // {id,label,zorunlu,deger,oto,ph,eksik,yes}
  ...
  return `<div class="ff-alan ff-trig ${dolu || phVar ? 'dolu' : ''}${o.eksik ? ' ff-eksik' : ''}${o.yes ? ' ff-deger-ok' : ''}" id="${o.id}">${val}${o.oto ? `<span class="ff-oto">${kacar(o.oto)}</span>` : ''}<span class="ff-ok">⌄</span><label>${kacar(o.label)}${o.zorunlu ? ' <span class="zor">*</span>' : ''}</label></div>`;
}
function ffTrigGuncelle(id, deger) {   // seçim sonrası tetik değerini güncelle (oto rozetini kaldırır)
  ...
  el.classList.toggle('dolu', !bos);
  const v = el.querySelector('.ff-val'); if (v) v.textContent = bos ? '' : deger;
  const o = el.querySelector('.ff-oto'); if (o) o.remove();
}
/* Alttan açılan seçici: arama + (grup/harf) + seç. secenekler: [{deger,ad,alt,ik,grup}] */
function altSecici(o) {
  const kap = document.createElement('div'); kap.className = 'altsec-perde';
  kap.innerHTML = `<div class="altsec" role="dialog">
      <div class="altsec-tut"></div>
      <div class="altsec-bas"><h3>${kacar(o.baslik || 'Seç')}</h3><button class="altsec-x" type="button" aria-label="Kapat">✕</button></div>
      ${o.arama ? `<div class="altsec-ara">${ik('ara')}<input type="text" id="asAra" placeholder="${kacar(o.placeholder || 'Ara…')}" autocomplete="off" autocorrect="off" spellcheck="false"></div>` : ''}
      <div class="altsec-liste" id="asListe"></div>
    </div>`;
  ...
  const ciz = (liste, ham) => {
    ...
    const eb = listeEl.querySelector('[data-ekle]'); if (eb) eb.onclick = () => { kapat(); if (o.onEkle) o.onEkle(ham.trim()); };
  };
  ciz(o.secenekler, '');
  if (o.arama) {
    const inp = kap.querySelector('#asAra');
    inp.addEventListener('input', () => { const ham = inp.value.trim(); const q = adNorm(ham); ciz(o.secenekler.filter(x => adNorm(x.ad).includes(q)), ham); });
...
/* Giriş yapan kullanıcının rolü: 'admin' | 'ortak' | 'kullanici' */
...
const YETKILER = [
  { grup: 'Görüntüleme', key: 'gor_baskaOrtak', ad: 'Diğer ortakların finansalı' },
...
function yetki(key) {
  ...
  const rol = girisRol();
  const vars = YETKI_VARSAYILAN[rol] || YETKI_VARSAYILAN.kullanici;
  const ust = (State.ayarlar && State.ayarlar.yetkiler && State.ayarlar.yetkiler[rol]) || {};
  return (key in ust) ? !!ust[key] : !!vars[key];
}
function hepsiniGor() { return adminMi() || yetki('gor_baskaOrtak'); }   // true → tüm ortakların verisi
function benId() { return aktifOrtakId(); }                    // giriş yapan ortağın id'si
/* "Ortakları göster" düğmesi kaldırıldı — görünürlük artık role/yetkiye bağlı (ortak yalnız kendini görür) */
...
async function veriYukle() {
  ...
  if (!State.giderler.some(g => adNorm(g.ad) === adNorm(NAKIT_CEKIM_KAT))) { await DB.ekle('giderler', { ad: NAKIT_CEKIM_KAT, grupId: null }); State.giderler = DB._oku('giderler'); }   // bankadan kasaya aktarım (masraf değil)
...
const MENU = [
  ...
  { grup: 'Muhasebe', ikon: 'muhasebe', ogeler: [
    { id: 'ice-aktar', ad: 'İçe Aktar', ikon: 'indir', baslik: 'İçe Aktar', izin: 'veri_iceAktar' },
    { id: 'gelirler', ad: 'Gelirler', ikon: 'gelir', baslik: 'Gelirler' },
    { id: 'hesap-defter', ad: 'Hesaplar', ikon: 'muhasebe', baslik: 'Hesaplar', izin: 'gor_hesaplar' },
...
const SAYFA_BASLIK = { 'rapor-karzarar-kontrol': 'Kâr-Zarar Kontrolü', 'gelirler': 'Gelirler', 'tanim-gider': 'Gider Kalemleri', 'tanim-komisyon': 'Kart Komisyon Oranları', 'ayar-firma': 'Firma Bilgileri', 'ayar-ortak': 'Kullanıcılar', 'ayar-yetki': 'Roller & Yetkiler', 'ayar-vergi': 'Vergi Oranları', 'ayar-hareket': 'Hareket Kaydı', 'ayar-arsiv': 'İçe Aktarma Arşivi', 'ayar-yedek': 'Yedekleme & Geri Yükleme', 'ayar-surum': 'Uygulama & Sürüm', 'ayar-acilis': 'Hesap Açılış Bakiyeleri', 'ayar-tema': 'Görünüm', 'ayar-rehber': 'Kullanım Rehberi', 'giderler': 'Giderler', 'ortaklar': 'Ortaklar', 'bekleyen': 'Bekleyen Tahsilatlar' };
// Tanımlamalar hub'ından açılan alt sayfalar (menüde 'Tanımlamalar' vurgulu kalsın)
const TANIM_ALT = ['rapor-karzarar-kontrol', 'ayar-firma', 'ayar-ortak', 'ayar-yetki', 'tanim-gider', 'tanim-komisyon', 'ayar-vergi', 'ayar-hareket', 'ayar-arsiv', 'ayar-yedek', 'ayar-surum', 'ayar-acilis', 'ayar-tema', 'ayar-rehber'];
...
function menuCiz() {
  ...
  for (const m of MENU) {
    ...
    if (m.grup) {
      const ogeler = m.ogeler.filter(o => !o.gizli && (!o.sadeceAdmin || adminMi()) && (!o.izin || yetki(o.izin)));
...
function egitmenKarlilik(donem) {
  ...
  const ortaklar = State.ortaklar.filter(o => o.aktif !== false && ortakMi(o) && ortakAyAktif(o, donem));   // o ay aktif ortaklar (katılım/ayrılış; Kullanıcı rolü hariç)
  ...
  const cikislar = [
    ...(State.bankaHareketleri || []).filter(b => b.yon === 'gider' && !bankaNakitCekimMi(b) && giderAitDonem(b) === donem).map(b => ({ kat: b.giderKategori, eg: b.egitmenId, tutar: Math.abs(Number(b.tutar) || 0), banka: true })),
...
SAYFALAR['karlilik'] = function karlilikSayfasi() {
  ...
  const imzali = (n) => n ? '−' + TL(n) : TL(0);   // gider/vergi satırı: boşsa 0
  const ortAdBul = id => { const o = State.ortaklar.find(x => x.id === id); return o ? o.ad : '—'; };
  const ortAv = (e) => { const o = State.ortaklar.find(x => x.id === e.id); return (o && o.foto) ? `<span class="kk-av"><img src="${o.foto}" alt=""></span>` : `<span class="kk-av bos">${basHarf(e.ad)}</span>`; };
  const dagitimEt = (m) => m.dagitim === 'havuz' ? '4 ortağa eşit' : m.dagitim === 'ortak' ? kacar(ortAdBul(m.hedefOrtakId)) : 'atanmadı';
  const gvOran = `%${Math.round(r.vOran * 100)}`;
  // Açılır/daraltılır kart: dar = Brüt Satış/POS/Giderler/Vergiler/Hakediş · genişlemiş = öngörü/tahakkuk detaylı
  const kartCiz = (e, rol) => {
    const id = e.id || ('m_' + adNorm(e.ad));
    const acik = karAcikSet.has(id);
    const vergiler = (e.kdvOngoru || 0) + (e.gvOngoru || 0);
    ...
    const tahSat = (girildi, pay, bekl) => girildi
      ...
      : `<div class="kk-vsat b"><span>Tahakkuk</span><b>⏳ ${bekl}</b></div>`;
    const dar = sat('Brüt Satış', TL(e.brut))
      + sat('POS Komisyonu', imzali(e.komisyon))
      + (e.maasliMi ? '' : sat('Giderler', imzali(e.giderPayi)))
      + sat('Vergiler', imzali(vergiler))
      + mahsupSat()
      + `<div class="kar-sat kk-hak"><span>Hakediş</span><b class="${hakCls}">${TL(hakGuncel)}</b></div>`;
    const genis = `
      <div class="kk-g"><div class="kk-gb">Brüt Satış</div>
        <div class="kar-sat"><span>Toplam</span><b>${TL(e.brut)}</b></div>
        <div class="kk-sub">Nakit ${TL(e.nakit)} · Havale ${TL(e.havale)} · Kart ${TL(e.kart)}</div></div>
      ...
      <div class="kk-g"><div class="kk-gb">Vergiler</div>
        ...
        ${tahSat(e.kdvTahakkukVar, e.kdvTahakkukPay, 'bekliyor')}
        <div class="kk-vad">Gelir Vergisi <span class="kk-mini">${gvOran}</span></div>
    ...
    return `<div class="kar-kart kk-kart2 ${e.maasliMi ? 'maasli' : ''} ${acik ? 'acik' : ''}">
      ...
      <div class="kk-govde">${acik ? genis : dar}</div>
    </div>`;
  };
...
function vergiTahakkukModal(donem, sonrasi) {
  ...
  $('#tkKaydet').onclick = async () => {
    ...
    State.vergiTahakkuk = DB._oku('vergiTahakkuk');
    modalKapat(); bildir('Vergi tahakkuku kaydedildi.', 'basari'); if (sonrasi) sonrasi();
  };
}
/* Bir ortağın vergi mahsuplaştırması dökümü (öngörü ↔ tahakkuk ↔ fark) */
function mahsupDetayModal(donem, ortakId) {
  const r = egitmenKarlilik(donem);
  const e = (r.ortaklar || []).find(x => x.id === ortakId); if (!e) return;
  const grup = (ad, ong, tvar, tpay, fark) => `<div class="md-g"><div class="md-gb">${ad}</div>
    <div class="md-sat"><span>Öngörü <span class="md-tag o">yazılan</span></span><b>−${TL(ong)}</b></div>
    ${tvar ? `<div class="md-sat"><span>Tahakkuk <span class="md-tag g">gerçek</span></span><b>−${TL(tpay)}</b></div>
      <div class="md-sat md-fark"><span>Fark → hakedişe</span><b class="${fark < 0 ? 'negatif' : 'poz'}">${fark >= 0 ? '+' : '−'}${TL(Math.abs(fark))}</b></div>`
      : `<div class="md-sat"><span>Tahakkuk</span><b class="md-bek">⏳ bekliyor</b></div>`}</div>`;
  const govde = `<div class="md-kisi">${kacar(e.ad)} · ${donemAdi(donem)}</div>
    ${grup('KDV', e.kdvOngoru, e.kdvTahakkukVar, e.kdvTahakkukPay, e.kdvFark)}
    ${grup('Gelir Vergisi', e.gvOngoru, e.gvTahakkukVar, e.gvTahakkukPay, e.gvFark)}
  ...
  const tabloCiz = (kayitlar) => {
    ...
    return `<div class="defter-bas">Tahsilat Defteri</div>
      <div class="ogr-tkart sade"><div class="ogr-kaydir"><table class="ogr-tablo sade iki-satir">
        <colgroup><col style="width:16%"><col style="width:32%"><col style="width:22%"><col style="width:30%"></colgroup>
...
function kartKomTutar(t, tip) {
  // Elle orantılı dağıtılmış komisyon (komisyonTutar) varsa onu kullan — orana göre hesaplamayı ez
  if (t && t.komisyonTutar != null && tip == null) return Number(t.komisyonTutar) || 0;
  return (Number(t.tutar) || 0) * tahsilatKomisyonOran('kart', tip || t.kartTipi) / 100;   // yuvarlama yok
}
// Banka komisyonunu (hedef) kart tahsilatlarına TUTARLARINA göre orantılı böler; toplam tam hedefe eşit (en büyük kalan yöntemi)
/* Ağırlıklar oranında hedef'i KURUŞ hassasiyetiyle böler; toplam tam = hedef (yuvarlama kaybı yok) */
function agirlikDagit(agir, hedef) {
  const top = agir.reduce((s, x) => s + (Number(x) || 0), 0);
  const H = Math.round((Number(hedef) || 0) * 100);
  if (top <= 0) { const p = agir.map(() => 0); if (p.length) p[0] = H; return p.map(x => x / 100); }
  const ham = agir.map(a => H * (Number(a) || 0) / top);
  const pay = ham.map(x => Math.floor(x));
  let kalan = H - pay.reduce((s, x) => s + x, 0);
  const sira = ham.map((x, i) => ({ i, f: x - Math.floor(x) })).sort((a, b) => b.f - a.f);
  for (let j = 0; j < kalan && sira.length; j++) pay[sira[j % sira.length].i] += 1;
  return pay.map(x => x / 100);
}
function komisyonOrantiliDagit(kartlar, hedef) {
  const top = kartlar.reduce((s, r) => s + (Number(r.tutar) || 0), 0);
  if (top <= 0) return kartlar.map(() => 0);
  const H = Math.round((Number(hedef) || 0) * 100);   // kuruş — yuvarlama yok, toplam tam hedef
  const ham = kartlar.map(r => H * (Number(r.tutar) || 0) / top);
  const pay = ham.map(x => Math.floor(x));
  let kalan = H - pay.reduce((s, x) => s + x, 0);
  const sira = ham.map((x, i) => ({ i, f: x - Math.floor(x) })).sort((a, b) => b.f - a.f);
  for (let j = 0; j < kalan && sira.length; j++) pay[sira[j % sira.length].i] += 1;
  return pay.map(x => x / 100);   // TL (kuruş dahil)
}
...
/* Bir tahsilat GERÇEK gelir mi? Kural: nakit her zaman; kart/multinet/havale ancak
   bankaya yatınca (bir banka hareketinin eslesenIds'ine girince) gelir sayılır. */
function tahsilatGercek(t) {
  if (!t) return false;
  if (t.odemeTuru === 'nakit') return true;   // nakit anında gelir (kasaya girer)
  const id = String(t.id);
  return (State.bankaHareketleri || []).some(k => Array.isArray(k.eslesenIds) && k.eslesenIds.map(String).includes(id));   // yalnız banka eşleşmesi
}
function tahsilatUyum(t) { return tahsilatGercek(t); }   // Bekleyen sayfası + rozetler aynı kurala uyar
...
function ttSatirHTML(t) {
  ...
  return `<button type="button" class="ttl-sat" data-ttduz="${t.id}" data-gun="${kacar(t.tarih || '')}">
    <span class="ttl-us">
      ...
      <span class="ttl-tut mono">${TL(t.tutar)}</span>
    </span>
    <span class="ttl-alt">
...
SAYFALAR['mutabakat'] = function tahsilatTanimlaSayfasi() {
  ...
  ic().innerHTML = `
    <div class="tt-sayfa">
      <div class="tt-ust"><h3 class="tt-baslik">Tahsilat Defteri</h3><button type="button" class="btn btn-ana" id="ttYeni">${ik('yeni')} Yeni Tahsilat</button></div>
...
/* Öğrenci Adı seç — Plan4me üye adlarından ara; listede yoksa yazıp yeni ekle */
function tahsilatOgrSecModal(mevcutAd, onSec) {
  const pf = State.planformiTahsilat || [];
  ...
  const item = (r) => `<div class="ds-osat ${adNorm(r.ad) === adNorm(mevcutAd) ? 'sec' : ''}" data-ad="${kacar(r.ad)}"><span class="ds-ochk">${adNorm(r.ad) === adNorm(mevcutAd) ? '✓' : ''}</span><span class="oav-mini">${kacar((r.ad[0] || '?').toLocaleUpperCase('tr'))}</span><span class="ds-obil"><span class="ad">${kacar(r.ad)}</span><span class="alt">${r.eg ? kacar(r.eg) : 'Plan4me'} · ${r.sayi} kayıt</span></span></div>`;
  const govde = `
    <div class="uys-ara"><span class="ara-ik">${ik('ara')}</span><input type="text" id="toAra" placeholder="Öğrenci ara veya yeni ad yaz…" autocomplete="off" autocorrect="off" spellcheck="false"></div>
    <div class="to-ekle gizli" id="toEkle"></div>
    <div class="ds-oliste sec-liste-kaydir" id="toListe">${adlar.map(item).join('') || '<div class="gp-bos" style="margin:8px 6px">Henüz Plan4me kaydı yok. Adı yazıp ekleyin.</div>'}</div>`;
  const m = ustKatModal('Öğrenci Seç', `<span class="hr-rz-ik">${ik('ogrenci')}</span>Öğrenci`, govde, `<button class="btn" type="button" data-geri style="flex:1">‹ Geri</button>`);
  const sec = (ad) => { onSec(String(ad).trim()); m.kapat(); };
  const bindListe = () => m.qq('#toListe [data-ad]').forEach(el => el.onclick = () => sec(el.dataset.ad));
  const yenile = () => {
    const ham = (m.q('#toAra').value || '').trim();
    const q = adNorm(ham);
    const suz = adlar.filter(r => adNorm(r.ad).includes(q));
    m.q('#toListe').innerHTML = suz.map(item).join('') || '<div class="gp-bos" style="margin:8px 6px">Eşleşen yok.</div>';
    bindListe();
    const tam = adlar.some(r => adNorm(r.ad) === q);
    const ek = m.q('#toEkle');
  ...
  bindListe();
  m.q('#toAra').addEventListener('input', yenile);
...
function tahsilatTanimModal(mevcut, sonrasi) {
  ...
  const turGoster = () => { const m = ODEME_TURLERI_TT.find(x => x.id === ttForm.odemeTuru); const ek = ttForm.odemeTuru === 'kart' ? ` · ${kartTipiAd(ttForm.kartTipi)}` : ''; return (m ? m.ad : '') + ek; };
  const govde = `
    <div class="ff-alan ff-trig ff-tarih dolu"><span class="ff-val" id="hrTarihGos"></span><span class="ff-ok">⌄</span><label>Tarih <span class="zor">*</span></label><input type="date" id="hrTarih" class="ff-tarih-in" value="${(ttForm.tarih || bugunISO()).slice(0, 10)}"></div>
    ${ffTrig({ id: 'ttTurTrig', label: 'Ödeme Türü', zorunlu: true, deger: turGoster() })}
    ${ffInput({ id: 'ttTutar', label: 'Tutar', zorunlu: true, deger: ttForm.tutar ? binlik(ttForm.tutar) : '', inputmode: 'decimal' })}
    <div class="ff-grpbas">Eğitmen &amp; Öğrenci</div>
    ${ortaklar.length ? ffTrig({ id: 'ttEgTrig', label: 'Eğitmen', zorunlu: true, deger: ttForm.egitmenAd, oto: otoSen }) : `<div class="tt-egyok" style="margin-top:22px;color:var(--metin-soluk);font-size:13px">Önce Ayarlar › Ortak Bilgileri’nden eğitmen ekleyin.</div>`}
    ${ffTrig({ id: 'ttOgrTrig', label: 'Öğrenci Adı', zorunlu: true, deger: ttForm.ogrenciAd })}
    ${ffInput({ id: 'ttPk', label: 'Ders Paketi', zorunlu: false, deger: ttForm.dersPaketi })}`;
  const alt = `${duzenle ? `<button type="button" class="btn btn-kirmizi" id="ttSil" style="flex:0 0 auto">${ik('cop')}</button>` : ''}<button type="button" class="btn btn-ana ff-kaydet" id="ttKaydet">${ik('kaydet')} Kaydet</button>`;
  modalAc(duzenle ? 'Tahsilat Düzenle' : 'Tahsilat Ekle', govde, alt);
  $('#modalKap .modal').classList.add('modal-tam');
  tutarKutusuBagla($('#ttTutar'), ttForm.tutar || '');
  tarihGostergeBagla('#hrTarih', '#hrTarihGos');
  // Ödeme Türü
  $('#ttTurTrig').onclick = () => altSecici({ baslik: 'Ödeme Türü', secili: ttForm.odemeTuru,
    secenekler: ODEME_TURLERI_TT.map(x => ({ deger: x.id, ad: x.ad })),
    onSec: (v) => {
      ttForm.odemeTuru = v;
  ...
  if ($('#ttSil')) $('#ttSil').onclick = () => onayModal('Sil', 'Bu tahsilat tanımı silinsin mi?', async () => {
    ...
    modalKapat(); if (sonrasi) sonrasi(); else { const r = SAYFALAR[State.aktifSayfa]; if (r) r(); } bildir('Silindi.', 'basari');
  });
  $('#ttKaydet').onclick = async () => {
    ...
    State.tahsilatTanimlari = DB._oku('tahsilatTanimlari');
    modalKapat(); if (sonrasi) sonrasi(); else { const r = SAYFALAR[State.aktifSayfa]; if (r) r(); } bildir('Tahsilat tanımı kaydedildi.', 'basari');
  };
}
...
/* Türkçe büyük/küçük harf katlaması — I/İ/ı hepsi 'i'ye iner (banka BÜYÜK açıklaması ↔ kural eşleşsin) */
function trFold(s) { return String(s || '').replace(/[İIı]/g, 'i').toLowerCase().replace(/̇/g, ''); }
function giderKategoriOner(islem, aciklama) {
  const metin = trFold(String(islem || '') + ' ' + String(aciklama || ''));
  const kurallar = [...(State.kategoriKurallari || []), ...VARSAYILAN_KATEGORI_KURAL];   // kullanıcı kuralları öncelikli + varsayılanlar yedek
  for (const k of kurallar) { const a = trFold(k.anahtar || ''); if (a && metin.includes(a)) return k.kategori || ''; }
  return '';
}
...
function nakitGiderModal(mevcut, sonrasi) {
  ...
  const govde = `
    ...
    ${ffTrig({ id: 'nkgGiderTrig', label: 'Gider Adı', zorunlu: true, deger: nkGiderForm.giderAd })}
    ${ffInput({ id: 'nkgTutar', label: 'Tutar', zorunlu: true, deger: nkGiderForm.tutar ? binlik(nkGiderForm.tutar) : '', inputmode: 'decimal' })}
    <div class="ff-grpbas">Detay</div>
    ${ffTrig({ id: 'nkgEgTrig', label: 'İlgili Eğitmen', zorunlu: true, deger: egGoster() })}
  ...
  $('#nkgGiderTrig').onclick = () => {
    ...
    altSecici({ baslik: 'Gider Seç', arama: true, placeholder: 'Gider ara veya yaz…', harf: true, secili: nkGiderForm.giderId || '',
      secenekler: giderler.map(g => ({ deger: g.id, ad: g.ad })), onSec: secId, onEkle: ekle });
  };
  // İlgili Eğitmen
  ...
  $('#nkgDonemTrig').onclick = () => { const aylar = []; for (let i = 0; i < 15; i++) aylar.push(donemKaydir(buAy(), -i));
    altSecici({ baslik: 'Ait Olduğu Dönem', secili: nkGiderForm.donem || buAy(), secenekler: aylar.map(d => ({ deger: d, ad: donemAdi(d) })),
      onSec: (v) => { nkGiderForm.donem = v; ffTrigGuncelle('nkgDonemTrig', donemAdi(v)); } }); };
...
function arsivTarihTR(iso) { return (iso && iso.length >= 10) ? `${iso.slice(8, 10)}.${iso.slice(5, 7)}.${iso.slice(0, 4)}` : (iso || ''); }
function iaArsivCiz(g, isPf, yenile) {
  const tazele = yenile || iaTabCiz;
  const kol = isPf ? 'planformiTahsilat' : 'bankaHareketleri';
  const hepsi = (State[kol] || []);
  const turAd = isPf ? 'Plan4me aktarımı' : 'Banka aktarımı';
  // Toplu (batch) grupla — aktarimId'ye göre; etiketsiz eski kayıtlar tek "Önceki kayıtlar" grubunda
  const bmap = new Map();
  hepsi.forEach(r => {
    const bid = r.aktarimId || 'eski';
    let b = bmap.get(bid);
    if (!b) { b = { id: bid, eski: bid === 'eski', kisi: r.aktaranAd || '', tarih: r.aktarimTarih || '', kayitlar: [] }; bmap.set(bid, b); }
    b.kayitlar.push(r);
    if (!b.kisi && r.aktaranAd) b.kisi = r.aktaranAd;
    if ((r.aktarimTarih || '') > (b.tarih || '')) b.tarih = r.aktarimTarih;
  });
  const batchlar = [...bmap.values()].sort((a, b) => a.eski ? 1 : b.eski ? -1 : (b.tarih || '').localeCompare(a.tarih || ''));

  // --- DETAY görünümü: seçilen aktarımın kayıtları (salt görüntüleme) ---
  const acik = iaArsivDetay && batchlar.find(b => b.id === iaArsivDetay);
  if (acik) {
    const kayitSatir = isPf
      ? (r) => `<div class="ia-arow"><span class="ia-ar-t">${kacar(kisaTarih(r.tarih))}</span><span class="ia-ar-a">${kacar(r.egitmenAd || '')}${r.uyeAd ? ' · ' + kacar(r.uyeAd) : ''}</span><span class="ia-ar-x mono">${TL(r.tutar)}</span></div>`
      : (r) => `<div class="ia-arow"><span class="ia-ar-t">${kacar(kisaTarih(r.tarih))}</span><span class="ia-ar-a">${kacar(r.islem || '')}</span><span class="ia-ar-x mono ${(Number(r.tutar) || 0) < 0 ? 'negatif' : ''}">${TL(r.tutar)}</span></div>`;
    const list = acik.kayitlar.slice().sort((a, b) => (b.tarih || '').localeCompare(a.tarih || ''));
    g.innerHTML = `
      <div class="ia-arsiv">
        <button type="button" class="tnm-geri" id="arsDetGeri">‹ Aktarımlar</button>
        <div class="ars-det-bas">
          <div class="ars-det-t">${kacar(acik.eski ? 'Önceki kayıtlar' : turAd)}</div>
          <div class="ars-det-alt">${acik.eski ? '' : arsivTarihTR(acik.tarih) + (acik.kisi ? ' · ' + kacar(acik.kisi) : '') + ' · '}${acik.kayitlar.length} kayıt · salt görüntüleme</div>
        </div>
        <div class="ia-arliste">${list.map(kayitSatir).join('')}</div>
        ${adminMi() ? `<button type="button" class="btn btn-kirmizi btn-kucuk ars-batch-sil" id="arsBatchSil">${ik('cop')} Bu aktarımı sil</button>` : ''}
      </div>`;
    $('#arsDetGeri').onclick = () => { iaArsivDetay = null; tazele(); };
    { const bs = $('#arsBatchSil'); if (bs) bs.onclick = () => onayModal('Aktarımı Sil', `Bu aktarımdaki ${acik.kayitlar.length} kayıt silinecek. Emin misiniz?`, async () => {
      for (const r of acik.kayitlar) await DB.sil(kol, r.id);
      State[kol] = DB._oku(kol); iaArsivDetay = null; tazele(); bildir('Aktarım silindi.', 'basari');
    }); }
    return;
  }

  // --- LİSTE görünümü: her aktarım tek satır ---
  const batchSatir = (b) => `<div class="ars-bt" data-bt="${b.id}">
      <div class="ars-bt-ik ${isPf ? 'pf' : ''}">${isPf ? '📋' : '🏦'}</div>
      <div class="ars-bt-t">
        <div class="ars-bt-bas">${kacar(b.eski ? 'Önceki kayıtlar' : turAd)}</div>
        <div class="ars-bt-alt">${b.eski ? '' : `<span>${arsivTarihTR(b.tarih)}</span>${b.kisi ? `<span class="ars-bt-kisi">${kacar(b.kisi)}</span>` : ''}`}<span class="ars-bt-say">${b.kayitlar.length} kayıt</span></div>
      </div>
      <span class="ars-bt-ok">›</span>
    </div>`;
  g.innerHTML = `
    <div class="ia-arsiv">
      <div class="ia-ars-bas"><span>${isPf ? 'Plan4me' : 'Banka'} arşivi · <b>${hepsi.length}</b> kayıt · ${batchlar.length} aktarım</span></div>
      ${hepsi.length ? `<div class="ars-bt-liste">${batchlar.map(batchSatir).join('')}</div>` : `<div class="neon-bos">Bu kaynakta içe aktarılmış kayıt yok.</div>`}
      ${hepsi.length && adminMi() ? `<button type="button" class="btn btn-kirmizi btn-kucuk ars-tum-sil" id="iaTumSil">${ik('cop')} Tümünü Sil</button>` : ''}
    </div>`;
  $$('.ars-bt[data-bt]').forEach(b => b.onclick = () => { iaArsivDetay = b.dataset.bt; tazele(); });
  const ts = $('#iaTumSil'); if (ts) ts.onclick = () => onayModal('Tümünü Sil', `${isPf ? 'Plan4me' : 'Banka'} arşivindeki ${hepsi.length} kayıt silinecek. Emin misiniz?`, async () => {
    for (const r of (State[kol] || []).slice()) await DB.sil(kol, r.id);
    State[kol] = DB._oku(kol); iaArsivDetay = null; tazele(); bildir('İçe aktarılan kayıtlar silindi.', 'basari');
  });
}
  ...
  ic().innerHTML = `
    ...
    <div class="ia-sayfa">
      <div class="ia-ust"><div class="ia-seg">
        <button type="button" class="ia-oge ${arsivSekme === 'banka' ? 'sec' : ''}" data-ars="banka">Banka${bhN ? ` <span class="ia-rk">${bhN}</span>` : ''}</button>
        <button type="button" class="ia-oge ${arsivSekme === 'planformi' ? 'sec' : ''}" data-ars="planformi">Plan4me${pfN ? ` <span class="ia-rk">${pfN}</span>` : ''}</button>
      </div></div>
      <div id="arsGovde"></div>
    </div>`;
  $('#arsGeri').onclick = () => git('ayar-tanimlama');
  $$('.ia-oge[data-ars]').forEach(b => b.onclick = () => { if (arsivSekme !== b.dataset.ars) iaArsivDetay = null; arsivSekme = b.dataset.ars; SAYFALAR['ayar-arsiv'](); });
...
/* Plan4me kaydı ↔ Tahsilat Tanımla eşleşmesi: ad + tutar(±1TL) + tarih(±1 gün) */
function ttEslesme(p) {
  return (State.tahsilatTanimlari || []).find(t =>
    adNorm(t.ogrenciAd) === adNorm(p.uyeAd) &&
...
function bankaAciklamaKisa(a) {
  ...
  s = s.replace(/referans\s*:?/gi, ' ')
    .replace(/term\s*id\s*:?[^,]*/gi, ' ')
    .replace(/nolu\s+i[şs]yerinin[\s\S]*/i, ' ')     // "… nolu isyerinin \ 0301 nolu pos ve …" → tamamen at (batch açıklaması)
    .replace(/sicil\s*no\s*:?/gi, ' ')
    .replace(/ad\s*\/\s*[üu]nvan\s*:?/gi, ' ')
    .replace(/\bTR\d{2}[\d*\s]+/gi, ' ')             // maskeli IBAN
    .replace(/\bisl[\s\S]*/i, ' ')
    .replace(/\bpos\b/gi, ' ')
    .replace(/[\/\\|:]+/g, ' ')
    .replace(/\b[0-9A-F]{5,}\b/gi, ' ')              // uzun kod/referans
    .replace(/\b\d{4,}\b/g, ' ')                     // uzun sayı
    .replace(/\s{2,}/g, ' ').trim();
...
/* Banka hareketinin GÖSTERİLECEK açıklaması: önce kullanıcının yazdığı (kullaniciAciklama),
   yoksa karşı taraf kısa adı, yoksa not, yoksa kısa ham metin. Uzun ham metin asla gösterilmez. */
function bankaGosterAck(k) {
  if (!k) return '';
  const u = String(k.kullaniciAciklama || '').trim();
  if (u) return u;
  const kt = bankaKarsiTaraf(k); if (kt.ad) return kt.ad;
  const bn = bankaNot(k); if (bn) return bn;
  return bankaAciklamaKisa(k.aciklama) || k.islem || '';
}
function bankaAckKart(k) {   // Banka Açıklaması → küçük tek-satır kart (ham metin yok)
  const kt = bankaKarsiTaraf(k), bn = String(k.kullaniciAciklama || '').trim() || bankaNot(k);
  ...
  const sub = kt.ad ? `${kacar(k.islem || '')} · ${kacar(tarih)}` : `${kacar(tarih)} · Banka`;
  const notSatir = bn ? `<div class="bd-knot">📝 <b>${kacar(bn)}</b></div>` : '';
  return `<div class="bd-kart"><div class="bd-krow"><div class="bd-kl">${rozet ? `<span class="bd-rol ${rolCls}">${kacar(rozet)}</span>` : ''}<div class="bd-kad">${kacar(baslik)}</div><div class="bd-ksub">${sub}</div></div><div class="bd-ktut ${kir ? 'kir' : 'yes'} mono">${TL(k.tutar)}</div></div>${notSatir}</div>`;
}
...
/* Banka satırının ilgili tahsilat günü: batch/komisyon → 1 gün önce; diğer → aynı gün */
...
function bankaTahsilatEslesme(k) {
  ...
  if (k.yon === 'komisyon') {
    ...
    return { tip: 'komisyon', durum: (k.komisyonOnay && kartlar.length && Math.abs(komTop - Math.round(Math.abs(Number(k.tutar) || 0))) <= 1) ? 'ok' : 'yok', toplam: komTop };
  }
...
function iaOnizleCiz(kayitlar, dosyaAd) {
  ...
  if (isPf) {
    ...
    pfRows.sort((a, b) => (a.es ? 1 : 0) - (b.es ? 1 : 0));
    const esOk = pfRows.filter(r => r.es).length, esYok = pfRows.length - esOk;
    uyusmayan = esYok;
    // Üstte yalnız iki filtre düğmesi (sayaçlı) — basınca o grup listelenir
    ozet = `<div class="ia-flt">
      <button type="button" class="ia-flt-b yok ${iaOnFiltre === 'yok' ? 'sec' : ''}" data-flt="yok">Eşleşmeyen <b>${esYok}</b></button>
      <button type="button" class="ia-flt-b ok ${iaOnFiltre === 'ok' ? 'sec' : ''}" data-flt="ok">Eşleşen <b>${esOk}</b></button>
    </div>`;
    ...
    uyusmayan = yeni.reduce((n, k) => {   // uyuşmayan tahsilat (gelir/komisyon) + tanımsız gider → içe aktarımı engeller
      if (k.yon === 'gider') return n + (bankaGiderEsle(k) ? 0 : 1);
      if (k.yon === 'gelir' || k.yon === 'komisyon') { const es = bankaTahsilatEslesme(k); return n + (es && es.durum === 'ok' ? 0 : 1); }
  ...
  $('#iaKaydet').onclick = async () => {
    ...
    { const aktarimId = 'akt_' + Date.now().toString(36) + Math.random().toString(36).slice(2, 6);
      const ku = State.kullanici || {}; const benOrt = ku.ortakId ? State.ortaklar.find(o => o.id === ku.ortakId) : null;
      const aktaranAd = (benOrt && benOrt.ad) || ku.ad || 'Yönetici';
      const aktarimTarih = bugunISO();
      yeni.forEach(k => { k.aktarimId = aktarimId; k.aktaranAd = aktaranAd; k.aktarimTarih = aktarimTarih; k.aktarimTur = isPf ? 'planformi' : 'banka'; }); }
...
function giderTanimlaModal(k) {
  ...
  const sec = async (ad, gid) => {
    ...
    if (an) await DB.ekle('kategoriKurallari', { anahtar: an, kategori: ad, giderId: gid, yon: 'gider' });   // benzer açıklamalar otomatik eşleşsin
    State.kategoriKurallari = DB._oku('kategoriKurallari');
    m.kapat(); iaOnizleTazele(); bildir(`“${ad}” gideriyle eşleştirildi.`, 'basari');
  };
  const govde = `
...
/* Başka banka kayıtlarınca kullanılmış tahsilat id'leri (haric kaydınınkiler hariç) —
...
function bankaAdaylar(k) {
  ...
  return tanim.filter(t => t.odemeTuru !== 'nakit' && Math.round(gunFark(t.tarih, k.tarih)) === 0 && uygun(t));   // diğer gelir → aynı gün (nakit asla)
}
...
function bankaDetayModal(k, wiz) {
  ...
  const ortakAlan = () => { const o = ortaklar.find(x => x.id === f.egitmenId); return o
    ? ffTrig({ id: 'bdOrtak', label: 'İlgili Ortak', zorunlu: true, deger: o.ad })
    : `<div class="ff-alan ff-trig ff-eksik dolu" id="bdOrtak"><span class="ff-val ph">Ortak seçin</span><span class="ff-eksik-uyari">Zorunlu</span><span class="ff-ok">⌄</span><label>İlgili Ortak <span class="zor">*</span></label></div>`; };
  const komHazir = komisyonHazir(k);
  const komToplam = () => bankaIlgiliTahsilatlar(k).reduce((s, t) => s + kartKomTutar(t), 0);
  const komTam = () => !!k.komisyonOnay && komHazir && Math.abs(komToplam() - Math.abs(Number(k.tutar) || 0)) <= 1;
  const ogrToplam = () => f.eslesenIds.reduce((s, id) => { const t = (State.tahsilatTanimlari || []).find(x => String(x.id) === String(id)); return s + (t ? Number(t.tutar) || 0 : 0); }, 0);
  const komField = () => (!komHazir)
  ...
  const alanlarHTML = () => `
    ...
    ${ortaAlan()}
    ${f.yon === 'gider' && !nakitCekimMi(f.giderKategori) ? ffTrig({ id: 'bdEg', label: 'İlgili Eğitmen', deger: egGoster() }) : ''}
    ${dagilimHTML()}
  ...
  function bagla() {
    { const a = $('#bdAck'); if (a) a.oninput = () => { f.kullaniciAciklama = a.value; }; }   // kullanıcı açıklaması (yenile'de korunur)
    ...
    { const g = $('#bdGider'); if (g) g.onclick = () => {
        const list = (State.giderler || []).slice().sort((a, b) => (a.ad || '').localeCompare(b.ad || '', 'tr'));
        const sec = (ad, id) => { f.giderKategori = ad; if (id != null) k.giderKategoriId = id; yenile(); };
        const secId = (id) => { const gg = (State.giderler || []).find(x => x.id === id); if (gg) sec(gg.ad, gg.id); };
        const ekle = async (ham) => { const ad = ham.trim(); if (!ad) return; let gg = (State.giderler || []).find(x => adNorm(x.ad) === adNorm(ad)); if (!gg) { gg = await DB.ekle('giderler', { ad, grupId: null }); State.giderler.push(gg); } sec(gg.ad, gg.id); };
        altSecici({ baslik: 'Gider Seç', arama: true, placeholder: 'Gider ara veya yaz…', harf: true, secili: '', secenekler: list.map(gg => ({ deger: gg.id, ad: gg.ad })), onSec: secId, onEkle: ekle });
      } }
    ...
    { const e = $('#bdEg'); if (e) e.onclick = () => altSecici({ baslik: 'İlgili Eğitmen', arama: ortaklar.length > 6, secili: f.egitmenId || '', secenekler: [{ deger: '', ad: 'Genel (tüm ortaklar)' }, ...ortaklar.slice().sort((a, b) => a.ad.localeCompare(b.ad, 'tr')).map(o => ({ deger: o.id, ad: o.ad }))], onSec: (v) => { f.egitmenId = v || null; ffTrigGuncelle('bdEg', egGoster()); } }); }
    { const or = $('#bdOrtak'); if (or) or.onclick = () => altSecici({ baslik: 'İlgili Ortak', arama: ortaklar.length > 6, secili: f.egitmenId || '', secenekler: ortaklar.slice().sort((a, b) => a.ad.localeCompare(b.ad, 'tr')).map(o => ({ deger: o.id, ad: o.ad, alt: o.id === benId() ? 'Sen' : '' })), onSec: (v) => { f.egitmenId = v || null; yenile(); } }); }
...
function komisyonTuttur(k, onDone) {
  ...
  const hedef = Math.abs(Number(k.tutar) || 0);   // banka komisyonu — yuvarlama yok (kuruş dahil)
  ...
  let mod = komTutForm.some(r => r.dagit != null) ? 'dagit' : 'tip';
  let otoAyar = false, sorPay = false;   // sorPay: "bulunamadı — pay edelim mi?" bandı
  const toplam = () => mod === 'dagit' ? komTutForm.reduce((s, r) => s + (Number(r.dagit) || 0), 0) : komTutForm.reduce((s, r) => s + komTut(r.tutar, r.tip), 0);
  const tamMi = () => Math.abs(toplam() - hedef) <= 1;
  const ozetHTML = (top) => {
    const tut = Math.abs(top - hedef) <= 1; const fark = Math.abs(top - hedef);
    return `<div class="ktm-oz">
        <div class="ktm-r"><span class="ktm-lb">Banka Komisyonu</span><span class="ktm-v mono">${TL(hedef)}</span></div>
        <div class="ktm-r"><span class="ktm-lb">${mod === 'dagit' ? 'Dağıtılan' : 'Tanım Toplamı'}</span><span class="ktm-v mono ${tut ? 'ok' : 'yok'}">${TL(top)}${tut ? ' ✓' : ''}</span></div>
      </div>`;
  };
  const payHTML = () => sorPay ? `<div class="ktm-pay"><div class="t">Tam tutan kart tipi kombinasyonu <b>bulunamadı</b>. Komisyonu tahsilatlara <b>orantılı paylaştıralım</b> mı?</div><div class="btns"><button type="button" class="pv" id="ktPayVaz">Vazgeç</button><button type="button" class="pe" id="ktPayEvet">Evet, Pay Et</button></div></div>` : '';
  const govde = () => {
    const top = toplam();
    if (mod === 'dagit') {
      return `${ozetHTML(top)}${payHTML()}
        <div class="ktm-ipuc">Komisyon tahsilatlara orantılı paylaştırıldı. Kaydet'e bas.</div>
        <div class="ktm-liste">${komTutForm.map(r => `<div class="ktm-kart">
          <div class="ktm-ust"><span class="ktm-ogr">${kacar(r.ogrenciAd)}</span><span class="ktm-tut mono">${TL(r.tutar)}</span></div>
          <div class="ktm-dpay"><span>Komisyon payı</span><b class="mono">−${TL(Number(r.dagit) || 0)}</b></div>
        </div>`).join('')}</div>`;
    }
    return `${ozetHTML(top)}${payHTML()}
      ${(tamMi() && otoAyar) ? '<div class="ktm-rozbil">✓ Kart tipleri otomatik seçildi — tam tuttu</div>' : `<div class="ktm-ipuc">Kart tipini elle seç ya da “Kart Tipini Otomatik Seç”e bas.</div>`}
      <div class="ktm-liste">${komTutForm.map((r, i) => `<div class="ktm-kart">
        <div class="ktm-ust"><span class="ktm-ogr">${kacar(r.ogrenciAd)}</span><span class="ktm-tut mono">${TL(r.tutar)}</span></div>
        <div class="ktm-tipler" data-i="${i}">${TIP.map(x => `<button type="button" class="ktm-tc ${r.tip === x.id ? 'sec' : ''}" data-tip="${x.id}"><span class="ad">${x.ad}</span><span class="or">%${String(komTur(x.id)).replace('.', ',')} → ${TL(komTut(r.tutar, x.id))}</span></button>`).join('')}</div>
      </div>`).join('')}</div>`;
  };
  const cizGovde = () => { const g = $('#ktGovde'); if (g) g.innerHTML = govde(); bagla(); btnTazele(); };
  const bagla = () => {
    $$('#ktGovde .ktm-tipler .ktm-tc').forEach(b => b.onclick = () => { const i = +b.closest('.ktm-tipler').dataset.i; komTutForm[i].tip = b.dataset.tip; komTutForm[i].dagit = null; sorPay = false; cizGovde(); });
    { const pv = $('#ktPayVaz'); if (pv) pv.onclick = () => { sorPay = false; cizGovde(); }; }
    { const pe = $('#ktPayEvet'); if (pe) pe.onclick = () => { const pay = komisyonOrantiliDagit(komTutForm, hedef); komTutForm.forEach((r, i) => r.dagit = pay[i]); mod = 'dagit'; sorPay = false; kaydet(); }; }
  };
  const btnTazele = () => { const a = $('#ktAna'); if (a) a.innerHTML = tamMi() ? `${ik('kaydet')} Kaydet` : `${ik('onay')} Kart Tipini Otomatik Seç`; };
  async function kaydet() {
    // Komisyon her zaman BANKA TUTARINA (yuvarlama yok) tam eşit dağıtılır:
    // tip modunda tip-komisyonu oranında, dağıt modunda tutara göre — kuruş hassas
    const agir = mod === 'dagit' ? komTutForm.map(r => Number(r.tutar) || 0) : komTutForm.map(r => komTut(r.tutar, r.tip) || 0);
    const w = agir.some(x => x > 0) ? agir : komTutForm.map(r => Number(r.tutar) || 0);
    const pay = agirlikDagit(w, hedef);
    for (let i = 0; i < komTutForm.length; i++) {
      const r = komTutForm[i]; const t = State.tahsilatTanimlari.find(x => x.id === r.id); if (!t) continue;
      await DB.guncelle('tahsilatTanimlari', r.id, { komisyonTutar: pay[i], kartTipi: r.tip || t.kartTipi || null, komisyonOran: r.tip ? tahsilatKomisyonOran('kart', r.tip) : (t.komisyonOran != null ? t.komisyonOran : null) });
    }
    State.tahsilatTanimlari = DB._oku('tahsilatTanimlari');
    k.komisyonOnay = true;   // Komisyon Eşleştir'de onaylandı
    if (k.id) { await DB.guncelle('bankaHareketleri', k.id, { komisyonOnay: true }); State.bankaHareketleri = DB._oku('bankaHareketleri'); }
    iaOnizleTazele(); bildir('Komisyon ayarlandı.', 'basari');
    kapat();   // Hareket Detayı'na geri dön
  }
  const otomatikSec = () => {
    const kombo = komisyonKombinasyonAra(komTutForm, hedef);
    if (kombo) { komTutForm.forEach((r, i) => r.tip = kombo[i]); mod = 'tip'; otoAyar = true; kaydet(); return; }   // bulundu → uygula + kaydet + dön
    sorPay = true; cizGovde();   // bulunamadı → aynı ekranda sor
  };
  modalTamAcVeyaGuncelle('Komisyon Eşleştir', `<div id="ktGovde">${govde()}</div>`,
    `<button type="button" class="btn btn-ana ff-kaydet" id="ktAna" style="flex:1">${ik('onay')} Kart Tipini Otomatik Seç</button>`);
  { const mdl = $('#modalKap .modal'); if (mdl) mdl.classList.remove('modal-wiz'); }
  { const kp = $('#modalKapat'); if (kp) kp.onclick = kapat; }   // ✕ ile de sihirbaza/detaya dön
  bagla(); btnTazele();
  $('#ktAna').onclick = () => { if (tamMi()) kaydet(); else otomatikSec(); };
}
...
function ttEslestirSec(p) {
  const govde = `<div class="tt-esl-sec">
    <div class="tt-esl-bilg"><b>${kacar(p.uyeAd)}</b> · ${TL(p.tutar)} · ${kacar(kisaTarih(p.tarih))}${p.egitmenAd ? ' · ' + kacar(p.egitmenAd) : ''}</div>
    <button type="button" class="btn btn-ana" id="teYeni">${ik('yeni')} Bu kayıttan yeni tanım</button>
    <button type="button" class="btn" id="teVar">${ik('onay')} Var olan tanımla eşleştir</button>
  </div>`;
  modalAc('Eşleştir', govde, `<button type="button" class="btn" id="teIptal" style="flex:1">Vazgeç</button>`);
  $('#teYeni').onclick = () => { modalKapat(); tahsilatTanimModal({ ogrenciAd: p.uyeAd, tutar: p.tutar, egitmenAd: p.egitmenAd || '', egitmenId: p.egitmenId || null, odemeTuru: p.tur || 'nakit', tarih: p.tarih }, iaOnizleTazele); };
  $('#teVar').onclick = () => { modalKapat(); ttVarSecModal(p); };
  $('#teIptal').onclick = modalKapat;
}
function ttVarSecModal(p) {
...
function neonAnaEkran() {
  const donem = buAy();
  const topla = (arr) => arr.reduce((s, p) => s + (Number(p.tutar) || 0), 0);
  const ttHam = (State.tahsilatTanimlari || []).filter(t => donemStr(t.tarih) === donem);   // dönemin tüm tahsilatları (defter)
  const tt = ttHam.filter(tahsilatGercek);   // gelir = yalnız gerçekleşen (nakit + bankaya yatmış)
  const bru = topla(tt);
  ...
  const gider = bh.filter(b => b.yon === 'gider' && !bankaNakitCekimMi(b)).reduce((s, b) => s + Math.abs(Number(b.tutar) || 0), 0)
    + (State.nakitGiderleri || []).filter(g => donemStr(g.tarih) === donem).reduce((s, g) => s + Math.abs(Number(g.tutar) || 0), 0);
  ...
  const bosVeri = !ttHam.length && !bh.length;
  const bekleyenTop = topla(ttHam.filter(t => !tahsilatGercek(t)));   // henüz bankaya yatmamış (bilgi amaçlı)
    ...
    <div class="neon-home">
      <div class="neon-ozet ${neonOzetAcik ? 'acik' : ''}" id="neonOzet">
        ...
        <div class="no-govde"><div class="no-ic">
          ...
          ${bekleyenTop ? ozetSat('Bekleyen (bankaya yatmadı)', TL(bekleyenTop), 'soluk') : ''}
...
function ortakFormu(mevcut) {
  ...
  const govde = `
    ...
    <div class="kf-blok kf-giris">
      ...
      <div class="gp-alan" style="margin:0"><label>Şifre ${mevcut && mevcut.sifreHash ? '(boş = değişmez)' : ''}</label><input type="password" class="gp-inp" id="oSif" placeholder="${mevcut && mevcut.sifreHash ? '•••••• (değiştirmek için yaz)' : 'En az 4 karakter'}" autocomplete="new-password"></div>
    </div>`;
  modalAc(mevcut ? 'Kişi Düzenle' : 'Yeni Kişi', govde,
  ...
  $('#oiKaydet').onclick = async () => {
    ...
    if (kul) {
      ...
      if (kulLc === SABIT_ADMIN.kullanici.toLocaleLowerCase('tr')) return bildir('Bu kullanıcı adı sistem yöneticisine ait.', 'hata');
      if (State.ortaklar.some(x => (!mevcut || x.id !== mevcut.id) && (x.girisAd || '').toLocaleLowerCase('tr') === kulLc)) return bildir('Bu kullanıcı adı başka kişide kullanılıyor.', 'hata');
      veri.girisAd = kul;
      const eskiHash = mevcut && mevcut.sifreHash;
      if (sif || !eskiHash) {
        if (sif.length < 4) return bildir('Şifre en az 4 karakter olmalı.', 'hata');
        veri.sifreHash = await sifreHash(sif);
      }
      if (!mevcut || mevcut.girisAktif === undefined) veri.girisAktif = true;
    } else {
      veri.girisAd = null; veri.sifreHash = null;   // giriş kaldırıldı
    }
...
SAYFALAR['ayar-ortak'] = function () {
  ...
  const kart = (o, i) => {
    const av = o.foto ? `<img src="${o.foto}" alt="">` : kacar(basHarfKisi(o.ad));
    const rol = o.rol || 'ortak';
    ...
    return `<div class="ku">
      ...
      <div class="ku-t">
        <div class="ku-ad">${kacar(o.ad)} <span class="rz rz-${rol}">${ROL_AD[rol]}</span>${!aktif && tanimli ? ' <span class="ku-kapali">giriş kapalı</span>' : ''}</div>
        <div class="ku-alt">${parca.join(' · ')}</div>
      </div>
      ${tanimli ? `<span class="ku-sw ${aktif ? 'on' : ''}" data-kulakt="${o.id}" title="Giriş aktif"><i></i></span>` : ''}
      <button type="button" class="ku-ed" data-duzenle="${o.id}" title="Düzenle">✎</button>
      <button type="button" class="ku-ed ku-sil" data-sil="${o.id}" title="Sil">🗑️</button>
    </div>`;
  };
  ic().innerHTML = `
    <div class="tnm-scr-ust">
      <button type="button" class="tnm-geri" id="obGeri">‹ Ayarlar</button>
      <button type="button" class="gp-ekle" id="ortEkle">＋ Ekle</button>
    </div>
    <div class="ku-list">
      <div class="ku kadmin">
        <div class="ku-av a1">${kacar(basHarfKisi(SABIT_ADMIN.kullanici))}</div>
        <div class="ku-t"><div class="ku-ad">Sistem Yöneticisi <span class="rz rz-admin">Admin</span></div>
          <div class="ku-alt">Kullanıcı: <b>${kacar(SABIT_ADMIN.kullanici)}</b> · kod ile tanımlı · değiştirilemez</div></div>
      </div>
      ${list.map(kart).join('') || '<div class="gp-bos">Henüz kişi yok. “＋ Ekle” ile başlayın.</div>'}
    </div>`;
  ...
  $$('[data-kulakt]').forEach(s => s.onclick = async () => {
    ...
    const yeni = o.girisAktif === false;
    await DB.guncelle('ortaklar', o.id, { girisAktif: yeni }); o.girisAktif = yeni;
    bildir(yeni ? 'Giriş açıldı.' : 'Giriş kapatıldı.', 'basari'); SAYFALAR['ayar-ortak']();
  });
  $$('[data-sil]').forEach(b => b.onclick = () => onayModal('Kişi silinsin mi?', 'Bu işlem geri alınamaz.', async () => {
    await DB.sil('ortaklar', b.dataset.sil);
    State.ortaklar = State.ortaklar.filter(o => o.id !== b.dataset.sil);
...
function ortakAyHesap(donem) {
  const aktif = State.ortaklar.filter(o => o.aktif !== false && ortakMi(o));
...
const TANIMLAR = [
  ...
  { id: 'rapor-karzarar-kontrol', ad: 'Kâr-Zarar Kontrolü', ikon: 'bolme', alt: 'Gelir ve giderlerin kâr dağıtımında nereye gittiği', grup: 'Muhasebe', sadeceAdmin: true, ortakDa: true },
...
function kzHesap(donem) {
  ...
  // GİRİŞLER — o ay girilen tüm tahsilatlar
  const tt = (State.tahsilatTanimlari || []).filter(t => donemStr(t.tarih) === donem);
  const girisTop = top(tt, t => t.tutar);
  const bekleyen = tt.filter(t => !tahsilatGercek(t)), bekTop = top(bekleyen, t => t.tutar);
  const gercekTop = girisTop - bekTop;
  if (!O.length && gercekTop) uyari.push({ ik: '⚠️', baslik: 'Bu ay aktif ortak yok', tutar: gercekTop, aciklama: 'Gelir ve giderler dağıtılacak ortak bulunamadığı için rapora girmiyor.' });
...
SAYFALAR['rapor-karzarar-kontrol'] = function () {
  ...
  const eksiTL = (n) => n ? '−' + TL(n) : TL(0);
  const pct = (n) => d.brut ? Math.max(0, Math.min(100, n / d.brut * 100)) : 0;
  const denge = !d.uyari.length && Math.abs(d.yuvarlama) <= 2;
  // 1) Kontrol rozeti
  const uyariTop = d.uyari.reduce((s, u) => s + Math.abs(u.tutar), 0);
  const rozet = denge
  ...
  // her satır: o → ortak değeri, h/m → grup değeri, toplam → şirket geneli (hep=false ise yalnız kendi sütunu + toplam)
  const satirlar = [
    { grp: 'Gelirler' },
    ...
    { ad: 'KDV öngörüsü', cls: 'r', o: o => -o.kdvOngoru, h: () => 0, m: () => -sM(m => m.kdvOngoru), t: -d.kdv },
  ...
  const kontrol = `<div class="kz-kutu"><h3>🔎 Para Kontrolü</h3><p class="kz-alt">Bu ay programa girilen her giriş ve çıkış nereye gitti</p>
    <div class="kz-pgrup"><div class="kz-pbas">⬇️ Girişler (tahsilat kayıtları)</div>
      ${sat('Programa girilen', TL(d.girisTop), 'bas')}
      ${sat('✓ Rapora dahil (gerçekleşen)', TL(d.gercekTop), 'g')}
...
const REHBER_BOLUM = [
  ...
  { em: '➕', emc: 'b2', nk: 'Gün içi 🎉', t: 'Tahsilat Ekle — ilk ödeme geldi!', img: 'tahsilat', toc: 'Tahsilat Ekle', tocAlt: 'Ödeme geldikçe deftere yaz',
    ...
    kural: { bas: 'Dikkat', p: '<b>Kart / havale</b> tahsilatı bankayla <b>eşleşmezse</b> gelir sayılmaz, raporlarda görünmez — dosyayı yükleyip eşleştirmeyi unutma 🔗' },
  ...
  { em: '📒', emc: '', nk: 'Ay ortası 🗓️', t: 'Hesaplar — küçük bir kasa turu', img: 'hesaplar', toc: 'Hesaplar', tocAlt: 'Kasa & banka defteri',
    hikaye: '<span class="who">💬 15 Ağustos 🗓️</span> “Bankada ne var, kasada ne kaldı acaba? 👀” diye merak ettin. Hesaplar sana minik bir kasa turu attırır 🚶‍♀️ — her kuruşun izini, nereye gidip nereden geldiğini gösterir 🔎',
    el: { l: 19, t: 10.5, e: '👇' }, etiket: 'Banka bakiyen 🏦', etL: 28, etT: 3.5,
    adimlar: ['Üstteki <span class="tus">Banka</span> 🏦, <span class="tus">Nakit</span> 💵, <span class="tus">Kart</span> 💳 kartlarından birine dokun', 'Aşağıdaki defterde her hareket ve sağında <b>yürüyen bakiye</b> sırayla akar 📈', 'Merak ettiğin bir satıra dokun, detayına bak 🔍'],
    ipuc: 'Başlangıç bakiyelerini <b>Ayarlar › Hesap Açılış Bakiyeleri</b>’nden bir kez girersin; defter oradan mutlu mutlu yürür 🎯' },
  ...
  { em: '💸', emc: 'b4', nk: 'Ay boyunca 🧾', t: 'Giderler — faturalar & paylaşım', img: 'giderler', toc: 'Giderler', tocAlt: 'Kira, elektrik, kâr dağıtımı',
    ...
    kural: { bas: 'Dikkat — sık yapılan hata', p: 'Ortağa ödeme yaparken kategoriyi <b>“Kâr Dağıtımı”</b> <b>seçmezsen</b>, program onu <b>normal masraf</b> sayar; ortağın hakedişinden <b>düşmez</b> ve genel gidere karışır.' },
  ...
  { em: '🧾', emc: 'b3', nk: 'Ay sonu ☎️', t: 'Vergi Tahakkuku — müşavir aradı', img: 'vergi', toc: 'Vergi Tahakkuku', tocAlt: 'Ay sonu gerçek vergi',
...
SAYFALAR['ayar-rehber'] = function () {
  ...
  const bol = (s, i) => `
    <div class="reh-bol" id="reh-sec-${i}">
      ...
      ${img[s.img] ? `<div class="reh-fig"><div class="reh-tel"><img src="${img[s.img]}" alt="${kacar(s.t)}"></div></div>` : ''}
      ...
      ${s.kural ? `<div class="reh-kural2"><span class="ik">⚠️</span><span class="kx"><span class="kb">${kacar(s.kural.bas || 'Dikkat')}</span>${s.kural.p}</span></div>` : ''}
...
/* -------- Ayarlar: Roller & Yetkiler (düzenlenebilir) — yalnızca admin -------- */
function yetkiDeger(rol, key) {
  const ust = (State.ayarlar && State.ayarlar.yetkiler && State.ayarlar.yetkiler[rol]) || {};
  return (key in ust) ? !!ust[key] : !!(YETKI_VARSAYILAN[rol] || {})[key];
}
SAYFALAR['ayar-yetki'] = function () {
  if (!adminMi()) { git('dashboard'); return; }
  // Çalışma taslağı — kaydedene kadar değişiklikler burada
  const taslak = { ortak: {}, kullanici: {} };
  YETKILER.forEach(y => { taslak.ortak[y.key] = yetkiDeger('ortak', y.key); taslak.kullanici[y.key] = yetkiDeger('kullanici', y.key); });
  const gruplar = [...new Set(YETKILER.map(y => y.grup))];
  const sw = (rol, key) => `<span class="yt-cell"><span class="sw ${taslak[rol][key] ? 'on' : ''}" data-rol="${rol}" data-key="${key}"><i></i></span></span>`;
  const satir = (y) => `<div class="yt-row"><span class="yt-ad">${kacar(y.ad)}</span>
      <span class="yt-cell"><span class="sw lock"><i></i></span></span>${sw('ortak', y.key)}${sw('kullanici', y.key)}</div>`;
  ic().innerHTML = `
    <div class="tnm-scr-ust"><button type="button" class="tnm-geri" id="ytGeri">‹ Ayarlar</button></div>
    <p class="tk-not">Ortak ve Kullanıcı rollerinin neyi görüp yapabileceğini buradan aç/kapa. <b>Admin</b> her şeye yetkilidir (kilitli). Kendi hakediş/özeti ve Görünüm her rolde açıktır.</p>
    <div class="yt-rolhead"><span class="yt-ad"></span>
      <span class="yt-cell"><span class="rc rc-admin">Admin 🔒</span></span>
      <span class="yt-cell"><span class="rc rc-ortak">Ortak</span></span>
      <span class="yt-cell"><span class="rc rc-kul">Kull.</span></span></div>
    ${gruplar.map(g => `<div class="yt-grup">${kacar(g)}</div>${YETKILER.filter(y => y.grup === g).map(satir).join('')}`).join('')}
    <button type="button" class="btn btn-ana yt-kaydet" id="ytKaydet">${ik('kaydet')} Yetkileri Kaydet</button>`;
  $('#ytGeri').onclick = () => git('ayar-tanimlama');
  $$('.sw[data-key]').forEach(s => s.onclick = () => {
    const rol = s.dataset.rol, key = s.dataset.key;
    taslak[rol][key] = !taslak[rol][key];
    s.classList.toggle('on', taslak[rol][key]);
  });
  $('#ytKaydet').onclick = () => {
    DB.ayarYaz({ ...State.ayarlar, yetkiler: taslak });
    bildir('Yetkiler kaydedildi.', 'basari');
    git('ayar-tanimlama');
  };
};
...
SAYFALAR['tanim-gider'] = function () {
  ...
  // Gruba neon renk kodu ata (Genel=neon, Kâr Dağıtımı=mor, kalanı ada göre sabit döngü)
  const grupAksan = (gAd) => {
    const n = adNorm(gAd);
    if (n === adNorm('Kâr Dağıtımı')) return { c: 'ga-dagit', ik: '💸' };
    if (n === adNorm('Genel Gider')) return { c: 'ga-genel', ik: '📁' };
    const pal = [{ c: 'ga-mavi', ik: '🏢' }, { c: 'ga-yesil', ik: '📦' }, { c: 'ga-amber', ik: '🧾' }];
    let s = 0; for (const ch of gAd) s += ch.charCodeAt(0);
  ...
  const bolumHTML = (gAd) => { const a = grupAksan(gAd); return `<div class="tnm-grup ${a.c}">
      <div class="tnm-grup-bas"><span class="rk">${a.ik}</span><span class="ad">${kacar(gAd)}</span><span class="say">${bucket[gAd].length}</span><span class="cizgi"></span></div>
...
/* “Tahsilatlar” (odemeler) sayfası kaldırıldı — tahsilatlar artık Gelirler + Tahsilat Defteri'nde görünür.
   Müşteri ödeme kayıtları için yardımcılar (FIFO düşüm / geri alma) korunuyor. */
...
/* Nakit kasa defteri: nakit tahsilatlar (tahsilatTanimlari) + nakit giderler (nakitGiderleri); yürüyen bakiye */
function nakitDefteri() {
  const rows = [];
  (State.tahsilatTanimlari || []).filter(t => t.odemeTuru === 'nakit').forEach(t => {
    // Tahsilat → üst: eğitmen adı, 2. satır: öğrenci adı
    rows.push({ kind: 'tahsilat', tarih: t.tarih, donem: donemStr(t.tarih), aciklama: t.egitmenAd || 'Tahsilat', altYazi: t.ogrenciAd || '', tutar: Number(t.tutar) || 0, ref: t, nakit: true });
  });
  (State.nakitGiderleri || []).forEach(g => {
    const ad = g.giderAd || g.kategori || g.aciklama || 'Nakit Gider';   // üst satır: gider adı
    const not = (g.aciklama && adNorm(g.aciklama) !== adNorm(ad)) ? g.aciklama : '';   // 2. satır: açıklama notu (addan farklıysa)
    rows.push({ kind: 'gider', tarih: g.tarih, donem: giderAitDonem(g), aciklama: ad, altYazi: not, ilgiliId: g.egitmenId || null, tutar: -(Number(g.tutar) || 0), ref: g, nakit: true });
  });
  // Bankadan nakit çekim → kasaya GİRİŞ (bankada çıkış olarak zaten görünür)
...
/* Nakit satırı (banka defteriyle aynı sade kart) — data-nrow: "kind:id" */
function nakitKart(h) {
  return `<div class="ia-satir" data-nrow="${h.kind}:${h.ref.id}">
...
function hesapHareketleri(hesap) {
  ...
  if (hesap === 'banka') {
    ...
    return bh.map(k => {
      ...
      // üst satır: gider adı / eğitmen adı — 2. satır: işlem/açıklama / öğrenci adı
      let ust, alt, dnm;
      if (k.yon === 'gider') {
        ust = k.giderKategori || YON_AD.gider;
        alt = bankaGosterAck(k);   // kullanıcı açıklaması / karşı taraf / kısa — uzun ham metin değil
        dnm = giderAitDonem(k);
      } else if (k.yon === 'komisyon') {   // komisyon → üst "Komisyon", alt: ilgili eğitmen(ler) ilk isimleri
        const ilg = bankaIlgiliTahsilatlar(k);
        const hoc = [...new Set(ilg.map(x => x.egitmenAd).filter(Boolean))];
        const ilkAd = ad => String(ad || '').trim().split(/\s+/)[0] || '';
        ust = 'Komisyon';
        ...
        const hoc = [...new Set(ilg.map(x => x.egitmenAd).filter(Boolean))];
        const ogr = [...new Set(ilg.map(x => x.ogrenciAd).filter(Boolean))];
        ust = hoc.length === 1 ? hoc[0] : (hoc.length > 1 ? hoc.length + ' hoca' : YON_AD[k.yon]);
        alt = ogr.length === 1 ? ogr[0] : (ogr.length > 1 ? ogr[0] + ' +' + (ogr.length - 1) : (bankaAciklamaKisa(k.aciklama) || k.islem || ''));
        dnm = donemStr(k.tarih);
      } else { ust = YON_AD[k.yon] || '—'; alt = bankaAciklamaKisa(k.aciklama) || k.islem || ''; dnm = donemStr(k.tarih); }
      return {
        tip: tutar >= 0 ? 'tahsilat' : 'gider', tarih: k.tarih, donem: dnm, ilgiliId: null,
        aciklama: ust, altYazi: alt, altTip: 'gr',
        tutar, bakiye: bak, banka: true, ref: k,
      };
    });
  }
  if (hesap === 'nakit') return nakitDefteri();   // nakit kasa: tahsilatlar(nakit) + nakit giderler
...
SAYFALAR['hesap-defter'] = function () {
  ...
  const satir = (h) => {
    ...
    const sub = h.altYazi ? kacar(h.altYazi) : '';   // 2. satır: açıklama / öğrenci
    const dp = String(h.donem || '').split('-'); const dnm = h.donem ? ((AY_KISA[(+dp[1] || 1) - 1] || '') + ' ' + String(dp[0] || '').slice(2)) : '';
    ...
    return `<div class="hs-row hs-tikla" ${rowAttr}>
      <span class="hs-c hs-c-tar"><span class="hs-tar-us">${kacar(kisaTarih(h.tarih))}</span>${dnm ? `<span class="hs-tar-dn">${dnm}</span>` : ''}</span>
  ...
  const kartMod = false;   // tüm hesaplar (Banka/Kasa/Kart) tabloda gösterilir
  const kartCiz = hesapAktif === 'nakit' ? nakitKart : bankaKart;
  const govdeCiz = () => {
  ...
  ic().innerHTML = `
    <div class="odeme-sayfa">
      ...
      ${tumList.length
        ? `<div class="hs-defter">
            <div class="hs-row hs-head"><span class="hs-c hs-c-tar">Tarih</span><span class="hs-c hs-c-ack">Gider / Eğitmen</span><span class="hs-c hs-c-tut">Tutar</span><span class="hs-c hs-c-bak">Bakiye</span></div>
  ...
  const rowBagla = () => {
    ...
    $$('[data-nrow]').forEach(b => b.onclick = () => nakitRowAc(b.dataset.nrow, hesapSayfayiYenile));
...
/* Bir giderin ait olduğu dönem: elle atanmış donem varsa o, yoksa tarihinden türet */
function giderAitDonem(g) { return (g && g.donem) ? g.donem : donemStr(g && g.tarih); }
/* "Ait Olduğu Dönem" <select> seçenekleri: buAy çevresinde -12…+3 ay (sel dahil) */
function donemSecenekleri(sel) {
  const arr = []; for (let i = 3; i >= -12; i--) arr.push(donemKaydir(buAy(), i));
  if (sel && !arr.includes(sel)) arr.unshift(sel);
  return arr.map(d => `<option value="${d}" ${d === sel ? 'selected' : ''}>${donemAdi(d)}</option>`).join('');
}
/* Bir dönemin gerçek giderleri (kârlılıktaki Genel Gider ile aynı kaynak): nakit + banka.
   Dönem = elle atanmış "ait olduğu dönem" (yoksa tarihten). */
function giderKalemleriDonem(donem) {
  const out = [];
  (State.nakitGiderleri || []).filter(g => giderAitDonem(g) === donem).forEach(g => {
    out.push({ tarih: g.tarih, donem: giderAitDonem(g), ad: (g.giderAd || g.kategori || g.aciklama || 'Gider'), tutar: Math.abs(Number(g.tutar) || 0), kaynak: 'nakit', egitmenId: g.egitmenId || null, aciklama: g.aciklama || '', ref: g });
  });
  (State.bankaHareketleri || []).filter(b => b.yon === 'gider' && !bankaNakitCekimMi(b) && giderAitDonem(b) === donem).forEach(b => {   // nakit çekim masraf değil
    const ad = (b.giderKategori || '').trim() || bankaAciklamaKisa(b.islem) || b.islem || 'Gider';
    out.push({ tarih: b.tarih, donem: giderAitDonem(b), ad, tutar: Math.abs(Number(b.tutar) || 0), kaynak: 'banka', egitmenId: b.egitmenId || null, aciklama: bankaGosterAck(b), ref: b });
  });
  return out.sort((a, b) => (b.tarih || '').localeCompare(a.tarih || ''));
}
let giderGorunum = 'tablo';   // 'rapor' | 'tablo' (ilk açılış: tablo)
let giderDonem = null;
let giderLimit = 25;          // tablo: gösterilen satır sayısı
let giderAcik = new Set();   // rapor: açık düğümler (g:grup / k:grup:gider) — varsayılan hepsi kapalı
SAYFALAR['giderler'] = function giderlerSayfasi() {
  ...
  const nakitTop = kalemler.filter(k => k.kaynak === 'nakit').reduce((s, k) => s + k.tutar, 0);
  const bankaTop = kalemler.filter(k => k.kaynak === 'banka').reduce((s, k) => s + k.tutar, 0);
  const kaynakAd = (k) => k === 'nakit' ? 'Nakit' : 'Banka';
  ...
  // RAPOR: Gider Grubu → gider kalemi → kayıtlar (hepsi kapalı başlar; 0 dahil tüm tanımlı giderler)
  const raporCiz = () => {
    const recMap = {}; kalemler.forEach(k => { (recMap[k.ad] || (recMap[k.ad] = [])).push(k); });
    const totOf = (ad) => (recMap[ad] || []).reduce((s, k) => s + k.tutar, 0);
    const bucket = {}; const seen = new Set();
    const ekle = (grpAd, ad) => { const b = bucket[grpAd] || (bucket[grpAd] = { ad: grpAd, kalemler: [], toplam: 0 }); b.kalemler.push({ ad, tutar: totOf(ad), kayitlar: recMap[ad] || [] }); b.toplam += totOf(ad); };
    (State.giderler || []).forEach(g => { const ad = (g.ad || '').trim(); if (!ad) return; seen.add(adNorm(ad)); ekle(giderGrupAd(g.grupId) || 'Genel Gider', ad); });
    Object.keys(recMap).forEach(ad => { if (!seen.has(adNorm(ad))) ekle('Diğer', ad); });
    const gruplar = Object.values(bucket);
    if (!gruplar.length) return `<div class="gp-bos">Önce Tanımlamalar › Giderler'den gider ekleyin.</div>`;
    gruplar.sort((a, b) => (a.ad === 'Genel Gider' ? -1 : b.ad === 'Genel Gider' ? 1 : b.toplam - a.toplam));
    const grpHTML = gruplar.map(g => {
      const gkey = 'g:' + g.ad, gAcik = giderAcik.has(gkey);
    ...
    return `<div class="gr-kutu">${grpHTML}<div class="gr-dip"><span class="k">${ik('para')} Toplam Gider</span><span class="v">−${binlik(toplam)} ₺</span></div></div>`;
  };
  // TABLO: sade düz kayıt tablosu + "daha fazla yükle"
  const tabloCiz = () => {
    if (!kalemler.length) return `<div class="defter-bas">Masraf Defteri</div><div class="gp-bos">${donemAdi(donem)} için gider yok.</div>`;
  ...
  $$('#gdGovde [data-gacik]').forEach(b => b.onclick = () => { const a = b.dataset.gacik; if (giderAcik.has(a)) giderAcik.delete(a); else giderAcik.add(a); giderlerSayfasi(); });
...
function klavyeUyumKur() {
  ...
  const uygula = () => {
    ...
    // iOS klavye açılınca / yazarken sayfayı yukarı kaydırır (offsetTop) → sabit pencereler yukarı taşar ya da
    // aşağı iner. Uygulamada sayfa kaydırması hiç istenmez (body sabit) → kaydırmayı anında geri al.
    const vt = vv ? Math.round(vv.offsetTop) : 0;
    if (vt > 0 && editOdak()) { try { window.scrollTo(0, 0); } catch (_) { } }
...
async function girisYapOrtak(o) {
  State.kullanici = { email: (o.girisAd || o.id) + '@yogatugi', ad: o.ad, rol: o.rol || 'ortak', ortakId: o.id };
  oturumSakla();
  await uygulamayiBaslat();
}
...
function ustCubukKur() {
  ...
  { const gb = $('#geriBtn'); if (gb) gb.onclick = geriGit; }
  ...
  { const kr = $('#kmRehber'); if (kr) { kr.innerHTML = ik('belge') + '<span>Kullanım Rehberi</span>'; kr.onclick = () => { kulMenuKapat(); git('ayar-rehber'); }; } }
