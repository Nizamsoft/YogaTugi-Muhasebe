...
let gelirKapali = new Set();  // rapor: kapalı eğitmen grupları
...
SAYFALAR['gelirler'] = function gelirlerSayfasi() {
  ...
  const ozetHTML = `<div class="gg-ozet-satir"><span class="lbl">Brüt Satış</span><span class="val">${TL(bru)}</span></div>
    <div class="gg-dag"><div class="d"><span>Nakit</span><b>${binlik(nak)} ₺</b></div><div class="d"><span>Havale</span><b>${binlik(hav)} ₺</b></div><div class="d"><span>Kart</span><b>${binlik(krt)} ₺</b></div></div>`;
  // RAPOR: eğitmen (hoca) grubu — altında Banka Havale / Kart / Nakit toplamları (tıklanamaz)
  const raporCiz = () => {
    if (!kayitlar.length) return `<div class="gp-bos">${donemAdi(donem)} için gelir yok.</div>`;
    const gr = {};
    kayitlar.forEach(p => { const ad = (p.egitmenAd || '').trim() || 'Atanmamış'; const g = gr[ad] || (gr[ad] = { ad, toplam: 0, adet: 0, havale: 0, kart: 0, nakit: 0 }); g.toplam += Number(p.tutar) || 0; g.adet++; const t = Number(p.tutar) || 0; if (p.odemeTuru === 'havale') g.havale += t; else if (p.odemeTuru === 'kart' || p.odemeTuru === 'multinet') g.kart += t; else g.nakit += t; });
    const gruplar = Object.values(gr).sort((a, b) => b.toplam - a.toplam);
    const bolum = (etiket, tutar) => `<div class="gr-sat norow"><span class="ad"><span class="nk"></span>${etiket}</span><span class="gr-tut art">${tutar > 0 ? '+' : ''}${binlik(tutar)} ₺</span></div>`;
    const grpHTML = gruplar.map(g => {
      const acik = !gelirKapali.has(g.ad);
      const satirlar = acik ? (bolum('Banka Havale', g.havale) + bolum('Kart', g.kart) + bolum('Nakit', g.nakit)) : '';
      return `<div class="gr-grp"><div class="gr-grpbas" data-gkapa="${kacar(g.ad)}"><span class="ad"><span class="ik">${ik('kisi')}</span>${kacar(g.ad)}</span><span class="gr-cnt">${g.adet} tahsilat</span><span class="gr-tut top art">+${binlik(g.toplam)} ₺</span><span class="gr-chev">${acik ? '⌃' : '⌄'}</span></div>${satirlar}</div>`;
    }).join('');
  ...
  const tabloCiz = () => {
    ...
    return `<div class="defter-bas">Tahsilat Defteri</div>
      <div class="ogr-tkart sade"><div class="ogr-kaydir"><table class="ogr-tablo sade">
        <thead><tr><th>Tarih</th><th>Üye Adı</th><th>Ödeme Türü</th><th class="sag">Tutar</th><th>Eğitmen</th><th>Ders Adı</th></tr></thead>
        <tbody>${kesit.map(p => `<tr data-gelir="${p.id}"><td data-l="Tarih">${kacar(kisaTarih(p.tarih))}</td><td data-l="Üye Adı">${kacar(p.ogrenciAd || '—')}</td><td data-l="Ödeme Türü">${kacar(turAd(p.odemeTuru))}</td><td data-l="Tutar" class="sag mono">${TL(p.tutar)}</td><td data-l="Eğitmen">${kacar(p.egitmenAd || '—')}</td><td data-l="Ders Adı">${kacar(p.dersPaketi || '—')}</td></tr>`).join('')}</tbody>
      </table></div></div>
  ...
  ic().innerHTML = `
    <div class="kar-sayfa">
      <div class="gg-ust">
        <div class="gg-bol gg-row"><span class="gg-bas">Gelirler</span>${ayNavHTML(donem)}</div>
        <div class="gg-bol gg-ince"><div class="ia-seg gr-seg"><button type="button" class="ia-oge ${gelirGorunum === 'tablo' ? 'sec' : ''}" data-glr="tablo">Tablo</button><button type="button" class="ia-oge ${gelirGorunum === 'rapor' ? 'sec' : ''}" data-glr="rapor">Rapor</button></div></div>
        ${gelirGorunum === 'rapor' ? `<div class="gg-bol gg-ince gg-ozet">${ozetHTML}</div>` : ''}
      </div>
  ...
  $$('[data-gkapa]').forEach(b => b.onclick = () => { const a = b.dataset.gkapa; if (gelirKapali.has(a)) gelirKapali.delete(a); else gelirKapali.add(a); gelirlerSayfasi(); });
...
function tahsilatTanimModal(mevcut, sonrasi) {
  ...
  $('#ttOgrTrig').onclick = () => tahsilatOgrSecModal(ttForm.ogrenciAd, (ad) => {
    ...
    // Plan4me kaydından eğitmeni otomatik öner (eğitmen boşsa)
    if (!ttForm.egitmenAd) {
...
function nakitGiderModal(mevcut, sonrasi) {
  ...
  const egTrigIc = () => { const o = ortaklar.find(x => x.id === nkGiderForm.egitmenId); return trig(o ? `<span class="st-nm">${kacar(o.ad)}</span>` : `<span class="st-ph">Genel (tüm ortaklar)</span>`); };
  const govde = `
    ...
    <div class="gp-alan"><label class="gp-lbl-ik">${ik('belge')} Ait Olduğu Dönem</label><div class="gp-sel-sar"><select class="gp-sel" id="nkgDonem">${donemSecenekleri(giderAitDonem(nkGiderForm))}</select><span class="st-ok">▾</span></div></div>
    ...
    <div class="gp-alan"><label class="gp-lbl-ik">${ik('kisi')} İlgili Eğitmen (opsiyonel)</label><button type="button" class="sec-trig" id="nkgEgTrig">${egTrigIc()}</button></div>
  ...
  $('#nkgEgTrig').onclick = () => tahsilatSecModal('İlgili Eğitmen', 'kisi', [{ deger: '', ad: 'Genel (tüm ortaklar)' }, ...ortaklar.map(o => ({ deger: o.id, ad: o.ad }))], nkGiderForm.egitmenId || '', (v) => { nkGiderForm.egitmenId = v || null; $('#nkgEgTrig').innerHTML = egTrigIc(); });
  ...
  $('#nkgKaydet').onclick = async () => {
    ...
    const kayit = { tarih: ($('#nkgTarih').value || bugunISO()).slice(0, 10), donem: ($('#nkgDonem') && $('#nkgDonem').value) || null, giderAd: nkGiderForm.giderAd, giderId: nkGiderForm.giderId || null, tutar: Math.abs(tutar), egitmenId: nkGiderForm.egitmenId || null, aciklama: $('#nkgAcik').value.trim() };
...
function hesapHareketleri(hesap) {
  ...
  if (hesap === 'banka') {
    ...
    return bh.map(k => {
      ...
      let ilgiliAd;
      if (k.yon === 'gider') ilgiliAd = k.giderKategori || YON_AD.gider;
      else if (k.yon === 'gelir' || k.yon === 'komisyon') {   // tahsilat/komisyon → ilgili hoca(lar)
        const hoc = [...new Set(bankaIlgiliTahsilatlar(k).map(x => x.egitmenAd).filter(Boolean))];
        ilgiliAd = hoc.length === 1 ? hoc[0] : (hoc.length > 1 ? hoc.length + ' hoca' : YON_AD[k.yon]);
      } else ilgiliAd = YON_AD[k.yon] || '—';
      return {
        tip: tutar >= 0 ? 'tahsilat' : 'gider', tarih: k.tarih, ilgiliId: null, ilgiliAd,
...
SAYFALAR['hesap-defter'] = function () {
  ...
  const satir = (h) => {
    ...
    return `<tr>
      ...
      : h.nakit
      ? `<button type="button" class="duz-btn" data-nrow="${h.kind}:${h.ref.id}">${ik('kalem')} Düzenle</button>`
  ...
  const govdeCiz = () => {
    ...
    if (!list.length) return kartMod ? `<div class="gp-bos" style="margin:10px 4px">Eşleşen hareket yok.</div>` : `<tr><td colspan="7"><div class="gp-bos" style="margin:6px 4px">Eşleşen hareket yok.</div></td></tr>`;
...
SAYFALAR['giderler'] = function giderlerSayfasi() {
  ...
  const ozetHTML = `<div class="gg-ozet-satir"><span class="lbl">Genel Gider</span><span class="val">${TL(toplam)}</span></div>
    <div class="gg-dag"><div class="d"><span>Nakit</span><b>${binlik(nakitTop)} ₺</b></div><div class="d"><span>Banka</span><b>${binlik(bankaTop)} ₺</b></div></div>`;
  ...
  const tabloCiz = () => {
    ...
    return `<div class="defter-bas">Masraf Defteri</div>
      <div class="ogr-tkart sade"><div class="ogr-kaydir"><table class="ogr-tablo sade">
        <thead><tr><th>Tarih</th><th>İşlem Adı</th><th>Ödeme Türü</th><th class="sag">Tutar</th><th>İlgili Eğitmen</th><th>İlgili Dönem</th></tr></thead>
        <tbody>${kesit.map((k, i) => `<tr data-gk="${i}"><td data-l="Tarih">${kacar(kisaTarih(k.tarih))}</td><td data-l="İşlem Adı">${kacar(k.ad)}</td><td data-l="Ödeme Türü">${kaynakAd(k.kaynak)}</td><td data-l="Tutar" class="sag mono">${binlik(k.tutar)} ₺</td><td data-l="İlgili Eğitmen">${kacar(egAd(k.egitmenId))}</td><td data-l="İlgili Dönem">${kacar(donemAdi(k.donem || donemStr(k.tarih)))}</td></tr>`).join('')}</tbody>
      </table></div></div>
  ...
  ic().innerHTML = `
    <div class="kar-sayfa">
      <div class="gg-ust">
        <div class="gg-bol gg-row"><span class="gg-bas">Giderler</span>${ayNavHTML(donem)}</div>
        <div class="gg-bol gg-ince"><div class="ia-seg gr-seg"><button type="button" class="ia-oge ${giderGorunum === 'tablo' ? 'sec' : ''}" data-gdg="tablo">Tablo</button><button type="button" class="ia-oge ${giderGorunum === 'rapor' ? 'sec' : ''}" data-gdg="rapor">Rapor</button></div></div>
        ${giderGorunum === 'rapor' ? `<div class="gg-bol gg-ince gg-ozet">${ozetHTML}</div>` : ''}
      </div>
